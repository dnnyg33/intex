...
public class BusinessObjectDAO  {
  ...
  synchronized <T extends BusinessObject> T read(String id, Connection conn) throws Exception {
    ...
    // initialize the data of the object by stepping through its superclasses all the way to BusinessObject or 
    while (boClass != BusinessObject.class || Modifier.isAbstract(boClass.getModifiers())==false) {
  ...
  public <T extends BusinessObject> void initialize(T bo, Class boClass, Connection conn) throws Exception {
  ...
  /**Retrieves a string of the GUID for whatever table, given a where clause
 * @throws DataException 
 * @throws SQLException 
   * 
   */
  public ResultSet searchForRS(String boType, SearchCriteria... criteria) throws DataException, SQLException{
	  ResultSet rs=null;
...
//        else if (sc.getColumnValue() == null){
//        	
//        }
        else{
          throw new DataException("Invalid value in search criteria.  Type " + valueType.getName() + " is not supported by BusinessObjectDAO.");
